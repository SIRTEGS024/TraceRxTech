...
const DashboardNavbar = () => {
  ...
  useEffect(() => {
    ...
    if (user.loggedInAs?.companyId) {
      ...
      if (company) {
        ...
        companyName = user.loggedInAs.companyName || company.basicInfo?.companyName || '';
        companyEmail = company.basicInfo?.email || '';
      }
    } else if (user.role === 'exporter' || user.role === 'importer') {
      // Exporter/Importer logged in directly
      currentCompany = user;
      companyName = user.basicInfo?.companyName || '';
      companyEmail = user.basicInfo?.email || '';
    } else if (user.role === 'verifier' || user.role === 'freight agent') {
      // Agent logged in as themselves (not for a company)
  ...
  return (
    <motion.nav
      ...
      className="bg-gradient-to-r from-green-700 to-emerald-600 shadow-lg w-full"
    >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-3 sm:py-0 sm:h-16 space-y-3 sm:space-y-0">
          ...
          {companyInfo && (
            ...
            >
              {/* Company Name */}
              {companyInfo.name && (
                <motion.div className="text-white font-semibold text-sm sm:text-base text-center sm:text-right">
                  {companyInfo.name}
                </motion.div>
              )}
